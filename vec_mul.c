...
Datum vec_mul(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_mul);

/**
 * Multiply elements of two arrays.
 *
 * This function takes two arrays of n-elements and returns an array of
 * n-elements where each element is the multiplied result of the input elements
 * of the same index.
 */
Datum
vec_mul(PG_FUNCTION_ARGS)
{
  Oid elemTypeId;
  int16 elemTypeWidth;
  ...
  if (rightArray && elemTypeId != ARR_ELEMTYPE(rightArray)) {
    ereport(ERROR, (errmsg("vec_mul mins array must be the same type as input array")));
  }

  valsLength = (ARR_DIMS(leftArray))[0];
  if (rightArray && valsLength != (ARR_DIMS(rightArray))[0]) {
    ereport(ERROR, (errmsg("vec_mul right array must be the same length as left array")));
  }
  ...
  for (i = 0; i < valsLength; i++) {
    ...
    switch(elemTypeId) {
      ...
      case NUMERICOID:
        ...
        retContent[i] = DirectFunctionCall2(numeric_mul, leftContent[i], rightContent[i]);
#else
        retContent[i] = NumericGetDatum(numeric_mul_opt_error(DatumGetNumeric(leftContent[i]), DatumGetNumeric(rightContent[i]), NULL));
#endif
        break;
    }
  }
