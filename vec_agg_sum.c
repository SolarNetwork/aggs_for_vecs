...
Datum vec_agg_sum(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_agg_sum);

/**
 * Extract an array of sums from a VecAggStatsType.
 */
Datum
vec_agg_sum(PG_FUNCTION_ARGS)
{
  Datum result;
  ...
  for (i = 0; i < stats->nelems; i++) {
    if (stats->sums[i]) {
      result_build->dvalues[i] = stats->sums[i]; // TODO: need copy?
