...
Datum vec_div(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_div);

/**
 * Divide elements of two arrays.
 *
 * This function takes two arrays of n-elements and returns an array of
 * n-elements where each element is the quotient between the input elements
 * of the same index.
 */
Datum
vec_div(PG_FUNCTION_ARGS)
{
  Oid elemTypeId;
  int16 elemTypeWidth;
  ...
  if (ARR_NDIM(leftArray) > 1 || ARR_NDIM(rightArray) > 1) {
    ereport(ERROR, (errmsg("vec_div: one-dimensional arrays are required")));
  }
  ...
  if (elemTypeId != INT2OID &&
    ...
    ereport(ERROR, (errmsg("vec_div input must be array of SMALLINT, INTEGER, BIGINT, REAL, DOUBLE PRECISION, or NUMERIC")));
  }
  if (rightArray && elemTypeId != ARR_ELEMTYPE(rightArray)) {
    ereport(ERROR, (errmsg("vec_div mins array must be the same type as input array")));
  }

  valsLength = (ARR_DIMS(leftArray))[0];
  if (rightArray && valsLength != (ARR_DIMS(rightArray))[0]) {
    ereport(ERROR, (errmsg("vec_div right array must be the same length as left array")));
  }
      ...
      case NUMERICOID:
        if (DatumGetInt32(DirectFunctionCall2(numeric_cmp, rightContent[i], DirectFunctionCall1(int4_numeric, Int32GetDatum(0)))) == 0) {
          ...
          retContent[i] = DirectFunctionCall3(numeric_in, CStringGetDatum("NaN"),
								  ...
								  Int32GetDatum(-1));
#else
          retContent[i] = DirectFunctionCall3(numeric_in, 
          ...
          retContent[i] = DirectFunctionCall2(numeric_div, leftContent[i], rightContent[i]);
#else
          retContent[i] = NumericGetDatum(numeric_div_opt_error(DatumGetNumeric(leftContent[i]), DatumGetNumeric(rightContent[i]), NULL));
#endif
        }
        break;
    }
