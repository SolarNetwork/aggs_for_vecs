  ...
  for (i = 0; i < state->astate->alen; i++) {
    if (state->vec_counts[i]) {
      switch (state->astate->element_type) {
        ...
        case NUMERICOID:
          state->astate->dvalues[i] = DirectFunctionCall1(numeric_avg, state->vec_states[i]);
          ereport(NOTICE, (errmsg("avg %d = %s", i, DatumGetCString(DirectFunctionCall1(numeric_out, state->astate->dvalues[i])))));
          break;
