...
Datum vec_agg_mean(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(vec_agg_mean);

/**
 * Extract an array of means from a VecAggStatsType.
 */
Datum
vec_agg_mean(PG_FUNCTION_ARGS)
{
  Datum result;
  ...
  for (i = 0; i < stats->nelems; i++) {
    if (stats->means[i]) {
      result_build->dvalues[i] = stats->means[i]; // TODO: need copy?
